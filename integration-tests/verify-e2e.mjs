...
try {
  // 预设 model=paraformer（绕开 Radix Select 自动化不稳定）
  await page.addInitScript(() => {
    localStorage.setItem('voicetxt-model', 'paraformer')
  })
  log(`打开 ${URL}`)
  ...
  const hasOnboard = await page
    ...
    .catch(() => false)
  if (hasOnboard) {
    log('引导出现，关闭（测 sherpa 不下载 whisper）')
    await page.keyboard.press('Escape')
    await page.getByText('欢迎使用 voicetxt').waitFor({ state: 'hidden', timeout: 10000 })
  } else {
    log('未弹引导（模型已缓存）')
  }
  ...
  log('轮询队列状态（最多 10 分钟，sherpa 首次拉 CDN 较慢）…')
